...
public class PumpkinRaid {
	...
	private PumpkinRaid.Status status;
	private int celebrationTicks;
	private Optional<BlockPos> waveSpawnPos = Optional.empty();

	public PumpkinRaid(int p_i50144_1_, ServerWorld p_i50144_2_, BlockPos p_i50144_3_) {
		this.id = p_i50144_1_;
		...
		this.status = PumpkinRaid.Status.ONGOING;
		System.out.println("E");
	}

	public PumpkinRaid(ServerWorld p_i50145_1_, CompoundNBT p_i50145_2_) {
		...
		this.status = PumpkinRaid.Status.getByName(p_i50145_2_.getString("Status"));
		this.heroes.clear();
	...
	public boolean isStopped() {
		return this.status == PumpkinRaid.Status.STOPPED;
	}

	public boolean isVictory() {
		return this.status == PumpkinRaid.Status.VICTORY;
	}

	public boolean isLoss() {
		return this.status == PumpkinRaid.Status.LOSS;
	}
	...
	private Predicate<ServerPlayerEntity> getParticipantsPredicate() {
		return (p_221302_1_) -> {
			...
			return p_221302_1_.isAlive() && PumpkinRaidManager.get(this.world).findRaid(blockpos, 9126) == this;
		};
	}
	...
	public void stop() {
		...
		this.status = PumpkinRaid.Status.STOPPED;
	}

	public void tick() {
		if (!this.isStopped()) {
			if (this.status == PumpkinRaid.Status.ONGOING) {
				...
				if (!this.world.isVillage(this.center)) {
					if (this.groupsSpawned > 0) {
						this.status = PumpkinRaid.Status.LOSS;
					} else {
						this.stop();
					}
				}
				...
				if (this.isStarted() && !this.hasMoreWaves() && i == 0) {
					if (this.postRaidTicks < 40) {
						...
						this.status = PumpkinRaid.Status.VICTORY;
	...
	private void spawnNextWave(BlockPos p_221294_1_) {
		...
		for(PumpkinRaid.WaveMember raid$wavemember : PumpkinRaid.WaveMember.VALUES) {
...
//	}
	...
	private int getDefaultNumSpawns(PumpkinRaid.WaveMember p_221330_1_, int p_221330_2_, boolean p_221330_3_) {
		return p_221330_3_ ? p_221330_1_.waveCounts[this.numGroups] : p_221330_1_.waveCounts[p_221330_2_];
	}

	private int getPotentialBonusSpawns(PumpkinRaid.WaveMember p_221335_1_, Random p_221335_2_, int wave, DifficultyInstance p_221335_4_, boolean p_221335_5_) {
	...
	static enum Status {
		...
		private static final PumpkinRaid.Status[] VALUES = values();

		private static PumpkinRaid.Status getByName(String name) {
			for(PumpkinRaid.Status raid$status : VALUES) {
	...
	public static enum WaveMember implements net.minecraftforge.common.IExtensibleEnum {
		...
		private static PumpkinRaid.WaveMember[] VALUES = values();
		 ...
		 */
		public static PumpkinRaid.WaveMember create(String name, EntityType<? extends AbstractRaiderEntity> typeIn, int[] waveCountsIn) {
